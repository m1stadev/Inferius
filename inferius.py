...
def main():
    if args.ipsw:
        ...
        if args.verbose:
            print('[VERBOSE] Checking if device is A9...')
        is_a9 = ipsw.a9_check(firmware_bundle)
        if is_a9:
            if args.verbose:
                print('[VERBOSE] Device is A9, fetching correct board config...')
            ...
            if args.verbose:
                print('[VERBOSE] Device is not A9, continuing...')

        if args.verbose:
            ipsw.extract_ipsw(args.ipsw[0], 'yes')
        else:
            ipsw.extract_ipsw(args.ipsw[0])
        print('IPSW extracted! Verifying bootchain...')
        if args.verbose:
            ipsw.verify_bootchain(firmware_bundle, firm_bundle_number, 'yes')
        else:
            ipsw.verify_bootchain(firmware_bundle, firm_bundle_number)
        print('Bootchain verified! Patching bootchain...')
        if args.verbose:
            patch.patch_bootchain(firmware_bundle, firm_bundle_number, 'yes')
        else:
            patch.patch_bootchain(firmware_bundle, firm_bundle_number)
        ...
        if args.verbose:
            ipsw_name = ipsw.make_ipsw(firmware_bundle, 'yes')
        else:
            ipsw_name = ipsw.make_ipsw(firmware_bundle)
